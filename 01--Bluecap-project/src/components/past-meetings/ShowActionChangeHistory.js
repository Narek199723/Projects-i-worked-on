...
import { getSummary, storeActions } from "../../firebase/firestore";
import { Box, Button, CircularProgress } from "@material-ui/core";
import * as actions from "../../redux/actions";
import { connect } from "react-redux";

function ShowActionChangeHistory({
  open,
  onClose,
  meetingId,
  historyNo,
  ...
  useEffect(async () => {
    ...
    if (summaryData.history_actionPlans) {
      summaryData.history_actionPlans.forEach((h) => {
        summaryArry.push(h.value);
      });
    }

    if (summaryData.actionPlans) {
      summaryArry.push(summaryData.actionPlans);
    }
              ...
              >
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    // setSummaryHistoryNumber(summaryHistoryNumber + 1)
                    let payload = {
                      actionPlans: summaryHistory[summaryHistoryNumber],
                    };
                    let storesummaryStatus = await storeActions(
                      payload,
                      meetingId
                    );
                    if (storesummaryStatus) {
                      props.showSnackbar({
                        show: true,
                        severity: "success",
                        message: "Actions change restored successfully.",
                      });
                      setActionData(
                        summaryHistory[summaryHistoryNumber].split("-")
                      );
                      onClose(false);
                    } else {
                      props.showSnackbar({
